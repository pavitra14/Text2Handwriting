let textData = `Demo`;
let img, myFont;
// let fontssss = ['defaultText1', 'defaultText2', 'defaultText3'];
let change = 0;
...
function incrementor() {
	selected = $("#handwritings").val();
	change = fontssss.indexOf(selected);
	// console.log(change);
	changeFont();
}
	...
	dataAvailable.forEach((i) => {
		try {
			fontText['textImage' + String.fromCharCode(i)] = loadImage(
				str("static/trained/" + fontssss[change]) + '/' + str(i) + '_t.png'
			);
		} catch (error) {}
	});
	loop();
...
function loadPage() {
	img = loadImage('static/pages/page (2).jpg');
	loop();
}